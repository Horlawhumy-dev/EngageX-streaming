...
# Output the public IP of the ECS instance running the app (if exposed to the internet)
output "ecs_instance_public_ip" {
  description = "The public IP of the ECS instance running the app"
  value       = aws_ecs_service.django_service.network_configuration[0].assign_public_ip == "ENABLED" ? aws_ecs_service.django_service.network_configuration[0].subnet_ids[0] : null
}
